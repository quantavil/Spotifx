...
- **Player Store Bug Fixes & Refactor**: Fixed queue append bug (manual tracks insert right after current song), shuffle sort bug (restores originalQueue correctly), queue drag/drop error (removes by matching _qid), and zombie track bug (deleted queue items also removed from originalQueue). Refactored player store (`playTrack`, `playAll`, `playNext`, `addToQueue`) using tight helper functions `_setNewQueue` and `_insertManualTrack` to eliminate redundant logic.
- **Syntax and merge-conflict cleanup**: Fixed syntax errors and redundant duplicated blocks in `player.svelte.ts` and `+layout.svelte`, and resolved ts import extensions in `favorites.test.ts`.
- **Search bar & test environment fix**: Fixed search bar typing/debounce bug in Svelte 5 by untracking `internalValue` inside the sync effect. Configured Vitest browser resolution conditions in `vite.config.ts` and fixed simulated event bubbling in test.
- **Removed legacy bloat**: Uninstalled the unused `puppeteer` package, deleted legacy `test-ytmusic.ts` script, resolved `issue.md`, and removed empty `docs` directory.
- **Queue state consolidation**: Merged `desktopQueueOpen` and `queueOpen` into a single `queueOpen` flag (persisted to `spotifx-desktop-queue`). `toggleQueue()` now saves preference. Desktop + mobile share one toggle. `playTrack`/`playAll` auto-open queue on desktop (≥640px). `toggleFullScreen` no longer closes queue.
- **Player bar CSS fix**: Added missing `.player-bar` CSS rule to `MusicPlayer.svelte` — was causing the player to be invisible/unstyled.
- **Svelte 5 onDestroy crash**: Fixed a runtime error caused by store auto-subscription ($dynamicAmp) within the derived waveDataUrl and style property of PlayerProgressBar.svelte by subscribing in an effect and syncing to local state.
- **Vite Dev Server Import Conditions Fix**: Corrected vite.config.ts where resolve.conditions was overridden to [] during dev. This was causing Vite to serve the Server-Side Rendering (SSR) version of Svelte to the browser, leading to internal lifecycle (onDestroy) crashes when browser-only states were referenced.
- **Dynamic Expanded Player Layout**: Refactored the right offset on NowPlayingFull.svelte to dynamically update (with a smooth transition) based on whether the queue is open or closed, preventing empty black space on the right side of the screen when closing the queue while fullscreen.
- **Footer Removal**: Completely removed the footer from +layout.svelte containing Spotify disclaimers and data references.
