...
<div class="mb-5 animate-fade-in" style="animation-delay:40ms">
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
		<div class="flex items-center gap-4 flex-1 min-w-0">
			...
			{:else}
				<div
					class="flex-shrink-0 w-20 h-20 sm:w-24 sm:h-24 rounded-lg bg-white/10 flex items-center justify-center"
				>
					<span class="text-accent font-black text-2xl">#1</span>
				</div>
			{/if}
		...
		<div class="flex sm:flex-col items-center sm:items-end gap-3 sm:gap-2">
			...
			<div class="flex items-center gap-2">
				{#if track.spotifyId}
					<button
						onclick={toggleFav}
						class="w-8 h-8 flex items-center justify-center rounded-full transition-all cursor-pointer flex-shrink-0
